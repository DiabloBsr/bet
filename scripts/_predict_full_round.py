"""Round courant (ou [HH:MM]) — TOUS les matchs, Top-3 complet V5/V2/V6 + ENSEMBLE
(Top-1 = 1er du top-3). Montre λtot + total modal pour trancher. Standalone.
Usage: ./.venv/Scripts/python.exe scripts/_predict_full_round.py [HH:MM]"""
...
def main():
    ...
    print(f"now {now.astimezone(MG):%H:%M} | ROUND {TARGET} — TOUS les matchs, Top-3 V5/V2/V6 + ENSEMBLE\n")
    for r in ms.itertuples():
        ...
        tag=f"total modal {('6+' if mt==6 else mt)}"
        zone="🎯3buts" if mt==3 and 2.5<=lt<3.05 else ("⬇Under" if lt<2.45 else ("⬆Over" if lt>=3.3 else ""))
        print(f"┌ {r.team_a} v {r.team_b}  λtot {lt:.1f} | {tag} {zone}")
        print(f"│ V5  : {fmt(top5)}")
        print(f"│ V2  : {fmt(v2l)}")
        print(f"│ V6  : {fmt(v6['top'])}")
        print(f"│ ENS : {fmt(ens['top'])}   → Top1 {ens['modal']}")
        print("└"+"─"*62)
