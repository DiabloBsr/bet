"""SCAN LIVE — ensemble V5+V2+V6 : le score le plus probable consolidé par match
du round courant, + le modal de chaque moteur pour comparer. Standalone.
Usage: ./.venv/Scripts/python.exe scripts/_predict_ensemble_live.py [HH:MM]"""
...
from scraper.market_inversion import invert_markets
...
def main():
    ...
    print(f"now {now.astimezone(MG):%H:%M} | ROUND {TARGET} — ENSEMBLE V5+V2+V6 (score consolidé)\n")
    print(f"{'match':<31}{'λtot':>5}  {'V5':>5}{'V2':>5}{'V6':>5}  | {'ENSEMBLE Top-3':<26}")
    print("-"*86)
    for r in ms.itertuples():
        oh,od,oa=float(r.oh),float(r.od),float(r.oa)
        if oh<=1 or oa<=1: continue
        inv=invert_markets(oh,od,oa,r.extra_markets); lt=inv.lam_h+inv.lam_a
        ...
        try:
            ...
            v2t=[]
        ens=ensemble_from_raw(top5,v2t,v6,top_n=3)
        m5s=top5[0][0] if top5 else "?"; m2s=v2t[0][0] if v2t else "?"; m6s=v6["modal"]
        e3=" ".join(f"{sc}({p*100:.0f}%)" for sc,p in ens["top"])
        print(f"{r.team_a+' v '+r.team_b:<31}{lt:>5.1f}  {m5s:>5}{m2s:>5}{m6s:>5}  | {e3:<26}")
