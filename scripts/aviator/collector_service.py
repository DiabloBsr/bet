...
MINUTES = float(sys.argv[1]) if len(sys.argv) > 1 else 0     # 0 = continu (service)
    ...
    with sync_playwright() as p:
        ...
        while time.time() < deadline:
            ...
            if time.time() - last > 60:
                ...
                last = time.time()
            # re-nav / re-login si la page a été fermée ou le token expiré (~24h)
            try:
                if not any("aviator.studio" in (f.url or "") for f in page.frames):
                    page.goto(BET_URL, timeout=30000); time.sleep(3)
                    login(page)                          # ré-authentifie si déconnecté
                    for _ in range(2):
                        click_ok(page); time.sleep(2)
                    gu = next((f.url for f in page.frames if f.url and "aviator.studio" in f.url), None)
                    if gu:
                        game_url = gu
                        page.goto(game_url, timeout=30000); time.sleep(3); click_ok(page)
                        print("  ↻ session/jeu ré-ouvert", flush=True)
            except Exception as exc:
                print(f"  (reprise: {exc})", flush=True)
                time.sleep(15)
        ctx.close()
