"""Round courant (ou [HH:MM]) — tableau EN COLONNES, SEULEMENT V2 et V5 :
V2_T1, V2_Top3, V5_T1, V5_Top3, ENS(V2+V5)_T1, ENS_Top3, zone. Tous les matchs.
Usage: ./.venv/Scripts/python.exe scripts/_predict_cols_v2v5.py [HH:MM]"""
...
def main():
    ...
    print(f"now {now.astimezone(MG):%H:%M} | ROUND {TARGET} — V2 + V5 seulement\n")
    H=f"{'match':<26}{'V2_T1':>6}{'V2_Top3':>13}{'V5_T1':>6}{'V5_Top3':>13}{'ENS_T1':>7}{'ENS_Top3':>13}{'zone':>9}"
    print(H); print("-"*len(H))
    for r in ms.itertuples():
        ...
        p5=predict_match_v5(m5,r.team_a,r.team_b,oh,od,oa,extra_markets=r.extra_markets); v5l=p5.get("top5_scores_enriched") or []
        try:
        ...
        ens=ensemble_top_scores({sc:p for sc,p in v5l}, {sc:p for sc,p in v2l}, None, top_n=3)["top"]
        zone=("3buts" if mt==3 and 2.5<=lt<3.05 else ("Under" if lt<2.45 else ("Over" if lt>=3.3 else f"tot{mt}")))
        mn=(r.team_a+" v "+r.team_b)[:25]
        print(f"{mn:<26}{t1(v2l):>6}{t3(v2l):>13}{t1(v5l):>6}{t3(v5l):>13}{ens[0][0] if ens else '-':>7}{t3(ens):>13}{zone:>9}")
